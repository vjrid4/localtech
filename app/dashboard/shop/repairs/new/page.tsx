    ...
    <div className="space-y-4">
      ...
      {value ? (
        <div className="glass rounded-xl p-4 border border-accent-500/30 flex items-center justify-between">
          ...
          <button
            onClick={() => { setQuery(""); setResults([]); (onSelect as any)(null); }}
            className="text-xs text-graphite-500 hover:text-white transition px-3 py-1 rounded-lg hover:bg-white/5"
          >
            Change
          </button>
        </div>
        ...
        <>
          <div className="flex gap-2">
            <button
              ...
              className={`flex-1 py-2 rounded-lg text-sm font-medium border transition ${mode === "search" ? "bg-accent-500/15 border-accent-500/30 text-accent-400" : "border-white/10 text-graphite-400 hover:text-white"}`}
            >
              Search Existing
            </button>
            <button
              onClick={() => setMode("new")}
              className={`flex-1 py-2 rounded-lg text-sm font-medium border transition ${mode === "new" ? "bg-accent-500/15 border-accent-500/30 text-accent-400" : "border-white/10 text-graphite-400 hover:text-white"}`}
            >
              + New Customer
            </button>
          </div>
          ...
          {mode === "search" ? (
            <div className="space-y-3">
                ...
                <div className="space-y-2">
                  {results.map((c) => (
                    <button
                      key={c.id}
                      onClick={() => onSelect(c)}
                      className="w-full glass rounded-xl p-3 flex items-center gap-3 hover:border-accent-500/30 border border-transparent text-left transition"
                    >
                      <div className="w-9 h-9 rounded-full bg-graphite-700 flex items-center justify-center text-sm font-bold shrink-0">
                        {c.name[0]}
                      </div>
              ...
              {query.length > 1 && !searching && results.length === 0 && (
                <p className="text-graphite-500 text-sm px-1">
                  No customers found.{" "}
                  <button onClick={() => setMode("new")} className="text-accent-400 underline">Create new</button>
                </p>
              )}
            </div>
          ) : (
            <div className="space-y-3">
              ...
              <button
                ...
                className="w-full py-3 bg-accent-500 text-graphite-950 font-bold rounded-xl hover:bg-accent-400 transition disabled:opacity-40"
              >
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-graphite-400 text-sm">
          Pick from {customer.name}&apos;s devices or register a new one
        </p>
      </div>
          ...
          {devices.length > 0 && (
            <div className="flex gap-2 mb-1">
              <button
                ...
                className={`flex-1 py-2 rounded-lg text-sm font-medium border transition ${mode === "pick" ? "bg-accent-500/15 border-accent-500/30 text-accent-400" : "border-white/10 text-graphite-400 hover:text-white"}`}
              >
                Existing Devices
              </button>
              <button
          ...
          {mode === "new" && (
            <div className="space-y-3">
              <div>
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                  {POPULAR_BRANDS.map((b) => (
                    <button
                      key={b}
                      onClick={() => setBrand(b)}
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition ${brand === b ? "bg-accent-500/15 border-accent-500/30 text-accent-400" : "border-white/10 text-graphite-400 hover:text-white hover:bg-white/5"}`}
                    >
                      {b}
                    </button>
                  ))}
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition ${brand === "Other" ? "bg-accent-500/15 border-accent-500/30 text-accent-400" : "border-white/10 text-graphite-400 hover:text-white hover:bg-white/5"}`}
                  >
                    Other
                  </button>
                </div>
                {brand === "Other" && (
                  <input
                    ...
                    className="w-full px-4 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white placeholder-graphite-500 focus:outline-none focus:border-accent-500/50 text-sm"
                  />
                )}
              </div>
              <input
                ...
                placeholder="Model (e.g. iPhone 14 Pro, Galaxy S23) *"
                className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-graphite-500 focus:outline-none focus:border-accent-500/50 text-sm"
              />
              ...
              <button
                ...
                disabled={!(brand === "Other" ? customBrand : brand).trim() || !model.trim() || creating}
                className="w-full py-3 bg-accent-500 text-graphite-950 font-bold rounded-xl hover:bg-accent-400 transition disabled:opacity-40"
              >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs border transition ${
                form.issue === issue
                  ...
                  : "border-white/10 text-graphite-400 hover:text-white hover:bg-white/5"
              }`}
            >
              {issue}
            </button>
          ...
          {PRIORITY_OPTIONS.map((p) => (
            <button
              ...
              className={`py-3 rounded-xl border text-center transition ${
                form.priority === p.value ? "border-2" : "border"
              }`}
              style={
                form.priority === p.value
                  ? { background: p.color + "22", borderColor: p.color, color: p.color }
                  : { borderColor: "rgba(255,255,255,0.1)", color: "#808080" }
              }
            >
...
export default function NewRepairPage() {
  ...
  useEffect(() => {
    apiGet<{ success: boolean; data: ShopContext }>("/api/shop/context")
      .then((r) => {
        setContext(r.data);
        if (r.data.branches.length > 0) {
          setForm((f) => ({ ...f, branchId: r.data.branches[0].id }));
        }
      })
  ...
  const canNext = [
    !!form.customer,
    !!form.device,
    form.issue.trim().length > 3 && !!form.branchId,
    true,
  ][step];
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        <button
          onClick={() => router.push("/dashboard/shop/repairs")}
          className="text-graphite-500 hover:text-white transition text-sm"
        >
          ← Back
        </button>
      ...
      {step < 3 && (
        <div className="flex justify-between mt-4">
          <button
            ...
            disabled={step === 0}
            className="px-5 py-2.5 rounded-xl border border-white/10 text-graphite-400 hover:text-white hover:bg-white/5 transition disabled:opacity-30 text-sm"
          >
            Back
          </button>
          <button
            ...
            disabled={!canNext}
            className="px-6 py-2.5 rounded-xl bg-accent-500 text-graphite-950 font-bold hover:bg-accent-400 transition disabled:opacity-40 text-sm"
          >
            Continue →
          </button>
        </div>
