...
export default function RegisterPage() {
  ...
  async function handleBizEnquiry(e: React.FormEvent) {
    ...
    setBizError("");
    try {
      // POST as a CUSTOMER first, then flag for upgrade — admin reviews and provisions
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-16" style={{ background: "linear-gradient(135deg, #0a0a0a 0%, #0d1a10 50%, #0a0a0a 100%)" }}>
      <div className="w-full max-w-md">
        ...
        <div className="flex items-center gap-2 justify-center mb-8">
          ...
          <span className="text-xl font-bold text-white" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>LocalTech</span>
        </div>
        ...
        <div className="glass rounded-2xl p-8 border border-white/10">
          ...
          {tab === "customer" ? (
            ...
            <div className="text-center py-4 space-y-3">
              ...
              <p className="font-bold text-white">Enquiry received!</p>
              <p className="text-graphite-400 text-sm">Our team will reach out to {bizEmail} within 24 hours to set up your account.</p>
            </div>
          ) : (
                ...
                <div className="flex gap-2">
                  {(["REPAIR_SHOP_OWNER", "TECHNICIAN", "SUPPLIER"] as const).map((t) => (
                    <button
                      ...
                      className={`flex-1 py-2 rounded-lg text-xs font-medium border transition ${bizType === t ? "bg-accent-500/15 border-accent-500/30 text-accent-400" : "border-white/10 text-graphite-400 hover:text-white"}`}
                    >
